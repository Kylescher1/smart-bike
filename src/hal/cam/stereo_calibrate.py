...
def main():
    # Path where stereo_capture.py stores pairs
    project_root = os.path.join(os.path.dirname(__file__), "../../..")
    pairs_dir = os.path.join(project_root, "src/hal/cam/calibrate/data/stereo_pairs")
    data_dir = os.path.join(project_root, "src/hal/cam/calibrate/data")
    os.makedirs(data_dir, exist_ok=True)

    print(f"📁 Loading images from: {os.path.abspath(pairs_dir)}")
    ...
    left_images = sorted(glob.glob(os.path.join(pairs_dir, "*_left.png")))
    right_images = sorted(glob.glob(os.path.join(pairs_dir, "*_right.png")))

    if not left_images or not right_images:
        print("❌ No stereo image pairs found.")
        return
    ...
    print(f"\n💾 Saved fisheye calibration to {os.path.abspath(out_file)}")
    cv2.destroyAllWindows()
